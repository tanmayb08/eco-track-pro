...
export default function SearchPage() {
  ...
  const loadTools = async () => {
    ...
    let query = supabase
      ...
      .select(
        `
        id,name,category,description,price_per_day,location,condition,brand,model,is_available,owner_id,created_at,
        tool_images ( url )
      `
      )
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#f9fafb" }}>
      ...
      <main style={{ maxWidth: "1200px", margin: "0 auto", padding: "32px 16px" }}>
        <div style={{ marginBottom: "32px" }}>
          ...
          <p style={{ color: "#6b7280" }}>Find tools available in your community</p>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
          ...
          <div style={{ fontSize: "14px", color: "#6b7280" }}>{location ? `Near ${location}` : "All locations"}</div>
        </div>
